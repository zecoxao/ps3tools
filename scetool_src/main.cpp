...
static struct option options[] = 
{
	...
	{"program-vender-id", ARG_REQ, NULL, VAL_VENDER_ID},
	...
	{"np-drm-type", ARG_REQ, NULL, VAL_DRM_TYPE},
	...
	{"pkg-type", ARG_REQ, NULL, 'B'},
	{"pkg-id", ARG_REQ, NULL, 'C'},
	{"pkg-version", ARG_REQ, NULL, 'D'},
	{"pkg-attribute", ARG_REQ, NULL, 'E'},
	{"pkg-region", ARG_REQ, NULL, 'F'},
	{"pkg-image-offset", ARG_REQ, NULL, 'G'},
	{"pkg-chunk-offset", ARG_REQ, NULL, 'H'},
	{"pkg-current-chunk", ARG_REQ, NULL, 'I'},
	{"pkg-chunks-total", ARG_REQ, NULL, 'J'},
...
static void print_usage()
{
	...
	printf(" -0, --category          SELF/RVK/PKG/SPP      SCE File Category\n");
	...
	printf(" -3, --program-auth-id   e.g. 1010000001000003 Authority ID\n");
	printf(" -4, --program-vender-id e.g. 01000002         Vender ID\n");
	...
	printf(" -b, --np-drm-type       LOCAL/FREE            DRM Type\n");
	...
	printf(" -B, --pkg-type          1 to 8                Update package type.\n");
	printf(" -C  --pkg-id            64 bits value         Update package id.\n");
	printf(" -D  --pkg-version       e.g. 0003004100000000 Update package version.\n");
	printf(" -E  --pkg-attribute     32 bits value         Update package attribute.\n");
	printf(" -F  --pkg-region        32 bits value         Update package region.\n");
	printf(" -G  --pkg-image-offset  0(default)            Update package image offset.\n");
	printf(" -H  --pkg-chunk-offset  0(default)            Update package chunk offset.\n");
	printf(" -I  --pkg-current-chunk 1(default)            Update package current chunk.\n");
	printf(" -J  --pkg-chunks-total  1(default)            Update package chunks total.\n");
...
int main(int argc, char **argv)
{
	...
	//Try to set path from argument --data-path
	if (_data_path != NULL)
	{
		if(!access(_data_path, 0))
			ps3 = _data_path;
		else
		{
			printf("[*] Error: Incorrect Data path.\n");
			return 0;
		}
	}
	else
