...
static inline void _es_segment_ext_header(segment_ext_header_t *si)
{
	si->offset = _ES64(si->offset);
	si->size = _ES64(si->size);
	si->comp_algorithm = _ES32(si->comp_algorithm);
	si->unknown_0 = _ES32(si->unknown_0);
	si->unknown_1 = _ES32(si->unknown_1);
	si->encrypted = _ES32(si->encrypted);
}

static inline void _copy_es_segment_ext_header(segment_ext_header_t *dst, segment_ext_header_t *src)
{
	memcpy(dst, src, sizeof(segment_ext_header_t)); 
	_es_segment_ext_header(dst);
}
