...
bool np_decrypt_npdrm(sce_buffer_ctxt_t *ctxt)
{
	...
	else if(_ES32(np->drm_type) == NP_DRM_TYPE_FREE)
	{
	...
	else if(_ES32(np->drm_type) == NP_DRM_TYPE_LOCAL || _ES32(np->drm_type) == NP_DRM_TYPE_NETWORK )
	{
...
bool np_encrypt_npdrm(sce_buffer_ctxt_t *ctxt)
{
	...
	else if(_ES32(np->drm_type) == NP_DRM_TYPE_LOCAL)
	{
		if ((klicensee_by_content_id((s8 *)np->content_id, npdrm_key)) == FALSE)
			return FALSE;
	}
...
bool np_create_ci(npdrm_config_t *npconf, ci_data_npdrm_t *cinp)
{
	...
	//Can only create NPDRM SELF with "local" and "free" drm types.
	if(_klicensee_key != NULL)
		memcpy(npdrm_key, _klicensee_key, 0x10);
	else if(npconf->drm_type == NP_DRM_TYPE_FREE)
	{
		ks_np_klic_free = keyset_find_by_name(CONFIG_NP_KLIC_FREE_KNAME);
		if(ks_np_klic_free == NULL)
			return FALSE;
	...
	else if(npconf->drm_type == NP_DRM_TYPE_LOCAL)
	{
	...
	cinp->drm_type = _ES32(npconf->drm_type);
